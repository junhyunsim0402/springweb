...
public class MemberController3 {
    ...
    public ResponseEntity<?> login(
        ...
        if( result ) {
            ...
            // 2) 쿠키 옵션
            cookie.setHttpOnly(true);   // .setHttpOnly(true) : 쿠키접근 방법, true이면 js가 접근 못함
            cookie.setSecure(false);     // .setSecure(true) : true이면 https만 접근 가능
            cookie.setPath("/member");   // .setPath() : 쿠키 접근하는 경로, "/" : 전체경로
            // cookie.setMaxAge();         // setMaxAge(초) : 쿠키 유지시간

            // 3) 쿠키에 응답하기, response.addCookie(쿠키객체);
            response.addCookie(cookie);
            return ResponseEntity.ok(true);
        }else{return ResponseEntity.ok(false);} // 아니면 실패
    }

    // [3] 로그아웃 = 세션방식 --> 토큰방식 + 쿠키
    @GetMapping("/logout") public ResponseEntity<?> logout(HttpServletResponse response) {
        // 1) 매개변수에 HttpServletResponse response가져온다
        // 2) 삭제할 쿠키의 동일한 속셩명으로 null값 저장하는 쿠키 생성
    ...
    // [4] 마이페이지 = 세션방식 --> 토큰방식 + 쿠키
    @GetMapping("/my/info") public ResponseEntity<?> myInfo(@CookieValue(value = "token", required = false) String token){
        // @CookieValue : HTTP 요청의 cookie 정보 매핑
        // 1] @CookieValue("token") String token 매개변수로 받는다, required = false 설정하면 필수값이 아닌 상태
...
/*
    ...
    톰캣 세션
        ...
        VS
    쿠키
            : 브라우저에 저장, 보안낮음(JWT), 로그인/장바구니/자동로그인/사용자설정(테마)
        * TOKEN : 특정한 자료 암호화(JWT) 해서 인증

        활용)
            1) 세션은 서버에 저장하므로 * 보안 * 높지만 *대규모*서버에는 과부하 증가한다
            2) 토큰은 세션/쿠키 없이 HTTP 사용하지 않으므로 웹/앱 통합 가능하다.
            3) 쿠키는 브라우저에 저장하므로 *보안* 낮지만 JWT와 같이 사용하면 서버에 과부화 낮게 할 수 있다
