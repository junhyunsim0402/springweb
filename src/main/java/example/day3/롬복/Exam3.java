...
public class Exam3 {
    public static void main(String[] args) {
        ...
        // 클래스명.bulider().맴버변수명(값).맴버변수명(값).bulid()
        StudentDto studentDto3=StudentDto.builder().sno(1).sname("유재석").build();
        StudentDto studentDto4=StudentDto.builder().sname("강호동").sno(2).build();
        StudentDto studentDto5=StudentDto.builder().sname("심준현").build();
    }
}
@NoArgsConstructor        // 컴파일(코드번역될때) 기본 생성자 코드 자동 생성
@AllArgsConstructor       // 컴파일(코드번역될때) 전체 매개변수 생성자 코드가 자동 생성
