...
class SuperClass{void method(){}}   // 슈퍼 클래스
class SubClass extends SuperClass{  // 하위 클래스
    @Override void method(){}       // 상위 클래스로부터 물려받은 메소드 재정의 하는 어노테이션
...
class TestClass{
    @MyAnnotation( value1 = "주입하는 값")   // @어노테이션이란? 해당 기술 사용하기 위한 설명
    public void method(){ } // 메소드 : 만약에 해당 메소드가 실행되면 @myAnnotation에 포함된 기능들이 실행
}// method()가 실행될떄 RetentionPolicy.RUNTIME로 실행되면서 value1()가 실행
...
public class Exam1 {
    public static void main(String[] args) {
        ...
        * 2. 목적 : 1. 메타데이터 제공 2. 제공 간소화 3. 가독성 향상
        * 3. 종류
        *   1. 표준 어노테이션 : 자바에서 기본적으로 제공하는 어노테이션
        *       @overide : 상위 클래스로부터 메소드를 재정의
        ...
        *               @Target( ElementType.FIELD ) : 맴버변수 에서 사용
        *                   등등
        * 5. 스프링 어노테이션
        *   @ 어노테이션 활용 : 개발자는 단순한 어노테이션 전달 함으로써 내부적으로 복잡한 코드가 실행
        *   개발자 입장               스프링 프레임워크
